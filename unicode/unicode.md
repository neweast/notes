...
`unicode码点`数量超过65536后，码点的表示形式修改为了`U+[XX]XXXX`，X代表一个十六制数字，可以有4-6位，不足4位时，在前面补0，直到补足4位。超过4位后，该是几位就是几位。这样子之后，码点的范围变成了U+0000~U+10FFFF，码点数量的理论大小为`10FFFF+1=110000`（16进制），也就是 `（16 +１）* 65536`，一百万左右。为了更好分类管理如此庞大的码点数，把每65536个码点作为一个平面，总共17个平面。
